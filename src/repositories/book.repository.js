...
class BookFlightsRepository {
    constructor(options) {
        let { cosmosdb_name, cosmosdb_key, cosmosdb_url, database_name, local } = options;
        cosmosdb_url = cosmosdb_url  || `${cosmosdb_name}.documents.azure.com:10255`;
        database_name = database_name  || 'admin';
        const connectionString = local || `mongodb://${cosmosdb_name}:${cosmosdb_key}@${cosmosdb_url}/${database_name}?ssl=true&replicaSet=globaldb`;
